...
(function() {
  ...
  DataStoreRegistry = (function(_super) {
    ...
    DataStoreRegistry.prototype.get = function(id) {
      ...
      if (!(entry instanceof DataStoreModel)) {
        this.log.debug({
          entry: entry
        }, "creating " + this.entity.name + " using underlying entry");
        record = new this.entity.model(entry, {
          store: this.store,
          log: this.log
        });
        this.update(id, record);
        try {
          record.controller = new this.entity.controller(record, {
            log: this.log
          });
        } catch (_error) {
          err = _error;
          this.log.warn({
            method: "get",
            error: err
          }, "encountered issue while attaching controller to the new record!");
        }
      }
  ...
  DataStoreModel = (function(_super) {
    ...
    DataStoreModel.prototype.get = function() {
      ...
      enforce = function(x) {
        var err, id, inverse, record, results, val, validator, violations, _ref, _ref1;
        if (!enforceCheck) {
          return x;
        }
        ...
        val = (function() {
          ...
          switch (false) {
            ...
            case !((prop.model != null) && x instanceof Object):
              try {
                inverse = (_ref5 = prop.opts) != null ? _ref5.inverse : void 0;
                if (inverse != null) {
                  x[inverse] = this;
                }
  ...
  DataStoreController = (function(_super) {
    ...
    function DataStoreController(model, opts) {
      ...
      if (this.store.isReady) {
        ...
        this.store.once('ready', (function(_this) {
          return function() {
            try {
              return _this.create(opts != null ? opts.data : void 0);
            } catch (_error) {
              err = _error;
              _this.model.set('error', err);
              throw err;
            }
          };
        })(this));
