...
(function() {
  ...
  DataStoreController = (function(_super) {
    ...
    DataStoreController.prototype.afterSave = function() {
      return this.emit('save', [this.model.name, this.model.id]);
    };

    DataStoreController.prototype.update = function(data) {
      this.model.set('modifiedOn', new Date());
      return this.emit('update', [this.model.name, this.model.id]);
    };

    DataStoreController.prototype.destroy = function(data) {
      return this.emit('destroy', [this.model.name, this.model.id]);
    };
  ...
  DataStore = (function(_super) {
    ...
    DataStore.prototype.commit = function(record) {
      var entity, exists, registry, type;
      ...
      registry = ((function() {
        ...
        for (type in _ref) {
          entity = _ref[type];
          if (((_ref1 = entity.model) != null ? _ref1.prototype.constructor.name : void 0) === record.constructor.name) {
      ...
      return this.log.info({
        method: "commit",
        id: record.id
      }, "updated the store registry for %s", record.constructor.name);
    };
